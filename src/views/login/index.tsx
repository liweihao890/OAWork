  ...
  return (
    <section className="login-page">
      <a href={config.github.repositoryUrl} target="_blank">
        <svg width="56" height="56" viewBox="0 0 250 250" className="svg-wrapper" aria-hidden="true">
      ...
      <div className="wrap">
        <div>
          ...
          <Input.Group>
            <Input {...loginName} placeholder="Username" prefix={<Icon type="user" />} maxLength={32} autoComplete="off" onPressEnter={handleSubmit} />
            <Input {...password} placeholder="Password" prefix={<Icon type="lock" />} maxLength={32} type="password" autoComplete="off" onPressEnter={handleSubmit} />
            <Input {...code} placeholder="请输入验证码" prefix={<Icon type="picture" />} maxLength={4} autoComplete="off" onPressEnter={handleSubmit} suffix={<img src={`${api.getCaptcha}${captcha}`} className="captcha" onClick={reloadCaptcha} />} />
          </Input.Group>
          <Button type="primary" style={{ marginTop: "20px" }} size="large" loading={loading} block onClick={handleSubmit}>
            {loading ? "登 录 中..." : "登 录"}
          </Button>
          
          <div className="other-login">
            <span className="txt">其他登录方式</span>
            <Icon type="github" onClick={githubHandler} />
